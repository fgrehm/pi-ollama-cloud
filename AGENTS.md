...
- `RETIRED_MODEL_IDS` in `scripts/generate-models.ts` is intentionally empty: `GENERATED_MODELS` is a first-launch fallback and the live refresh surfaces whatever the API returns, so retired models are not curated out of the baked-in list.
...
Because the model catalog refreshes automatically at runtime, releases are only needed for pricing updates (`OLLAMA_TO_MODELSDEV` in `scripts/generate-pricing.ts`).
